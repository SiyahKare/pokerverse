...
function djb2Hex(seedHex: string): number {
  const s = seedHex.replace(/^0x/, '')
  let hash = 5381
  for (let i=0;i<s.length;i++) hash = ((hash << 5) + hash) ^ s.charCodeAt(i)
  return hash >>> 0
}
function* xs32(seedHex: string): Generator<number, void, unknown> {
...
export function deterministicShuffle(seedHex: string, deck: string[]) {
  const g = xs32(seedHex)
  const d = deck.slice()
  for (let i = d.length - 1; i > 0; i--) {
    const j = Number((g.next().value as number) % (i+1))
...
export function distributeWinnings(
  ...
  for (const pot of pots) {
    ...
    for (const seat of orderSeats(winners)) { if (rem===0n) break; perSeat[seat] = (perSeat[seat] ?? 0n) + 1n; rem -= 1n }
  }
